...
pub mod native {
    ...
    use rusb::{
        request_type, Device, DeviceHandle, Direction, GlobalContext, TransferType,
    };
    use thiserror::Error;
    ...
    impl BulkOnlyTransportDevice {
        ...
        pub fn command_in(
            ...
            self.handle.read_bulk(self.bulk_in, &mut csw_bytes, TIMEOUT).unwrap();
        ...
        pub fn command_out(
            ...
            self.handle.write_bulk(self.bulk_out, &cbw_bytes, TIMEOUT).unwrap();
            ...
            if let Some(data) = data {
                self.handle.write_bulk(self.bulk_out, data, TIMEOUT).unwrap();
            }
